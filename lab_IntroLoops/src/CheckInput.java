...
public class CheckInput {
	...
	public int getInt(String message) { // Method that checks to make sure that the input is an integer
		...
		while (!check) { // Checks to make sure that the input is and integer and won't continue unless
							// it is
			if (myobj.hasNextInt()) {
	...
	public int checkIntRange(String message, int lowPar, int highPar) { // Method that checks to make sure that the
																		// input is an integer in a certain range
		int output = 0;
		boolean check = false;
		boolean first = true;
		System.out.print(message);
		while (!check) { // Checks to make sure that the input is in a certain range and won't continue
							// unless it is
			if (myobj.hasNextInt()) {
				output = myobj.nextInt();
	...
	public String getMessage(String message, String desiredOutput) {
		...
		boolean check = false;
		System.out.println(message);
		while (!check) { // Checks to make sure that the input is a String and won't continue to the rest
							// of the code unless it is
			checker = myobj.next();
	...
	public String getYesNo(String message) {
		...
		while (!check) { // Checks to make sure that the input is a String and won't continue to the rest
							// of the code unless it is
			checker = myobj.next();
			if (checker.equals("y") || checker.equals("yes") || checker.equals("Yes") || checker.equals("n")
					|| checker.equals("no") || checker.equals("No")) {
				check = true;
				output = checker;
			} else {
				System.out.println("That wasn't a valid input");
